...
function [beta, alpha, beta1, innovation, P_out, P_pred_out, LL_out] = KalmanSlidingWindow123(price_eth, price_btc, Q, R, LL_in)
...
function [beta, alpha, beta1, spread, P_filtr,  P_pred_filtr, LL_nowy] = kalman_do_przodu(price_eth, price_btc, Q, R, length)
    ...
    for i = 1:length
        [beta(i), alpha(i), beta1(i), spread(i), P_filtr{i}, P_pred_filtr{i}, nowy_LL] = KalmanSlidingWindow123(price_eth(i), price_btc(i), Q, R, LL_petla);
        LL_petla = nowy_LL;
    end
    LL_nowy = LL_petla;
end

function [beta, alpha, beta1, P, P_cross, spread] = smoother(price_eth, price_btc, beta_in, alpha_in, beta1_in, P_pred_in, P_in, length) 
    ...
    P{length-1} = P_in{length-1};
    P_cross{length-1} = P_in{length}; %do poprawy, w teorii nieprawda, ale chyba good enough bo to tylko jedna próbka i nie ma znaczącego wpływu na wynik

    for i = length-1:-1:2
        C = P_in{i-1} * A' / P_pred_in{i};
        x = [beta_in(i); alpha_in(i); beta1_in(i)] + C * ([beta(i+1); alpha(i+1); beta1(i+1)] - A * [beta_in(i); alpha_in(i); beta1_in(i)]);
...
function [Q_nowe, R_nowe] = wyliczanie_QR(price_btc, beta, alpha, beta1, P, P_cross, spread, length)
    ...
    for i = 3:length-1
        ...
        C1 = C1 + P{i} + x*x';
        R = R + (spread(i)^2 + H * P{i} * H')/(length-4);   
    end

    Q_nowe = (C1-(B1/A1)*B1' + (B1/A1 - A)*A1*(B1/A1 - A)')/(length-4);
    R_nowe = R;
...
for krok = 1:100
    ...
    [beta_history, alpha_history, beta1_history, spread_history, P_history,  P_pred_history, LL_nowy1] = kalman_do_przodu(y_log, x_log, Q_start, R_start, length(x));
    ...
    [betasmooth_history, alphasmooth_history, beta1smooth_history, Psmooth_history, Pcross_history, spread_history] = smoother(y_log, x_log, beta_history, alpha_history, beta1_history, P_pred_history, P_history, length(x));
...
[beta_history2, alpha_history2, beta1_history2, P_pred_history2, Pcross_history2, spread_history2] = smoother(y_log, x_log, beta_history1, alpha_history1, beta1_history1, P_pred_history1, P_history1, length(x));
